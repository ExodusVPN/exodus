...
use crate::wire::{EthernetAddress, IpAddress, IpCidr, IpEndpoint};
...
impl<'a> Parser<'a> {
    ...
    fn try_<F, T>(&mut self, f: F) -> Option<T>
    ...
    fn accept_number(&mut self, max_digits: usize, max_value: u32,
        ...
        for _ in 1..max_digits {
            match self.try_(|p| p.accept_digit(hex)) {
                Some(digit) => {
    ...
    fn accept_mac(&mut self) -> Result<EthernetAddress> {
        if let Some(mac) = self.try_(|p| p.accept_mac_joined_with(b'-')) {
            return Ok(mac)
        }
        if let Some(mac) = self.try_(|p| p.accept_mac_joined_with(b':')) {
            return Ok(mac)
    ...
    fn accept_ipv6_part(&mut self, (head, tail): (&mut [u16; 8], &mut [u16; 6]),
        ...
        let double_colon = match self.try_(|p| p.accept_str(b"::")) {
        ...
        match self.try_(|p| p.accept_number(4, 0x10000, true)) {
    ...
    fn accept_ip(&mut self) -> Result<IpAddress> {
        #[cfg(feature = "proto-ipv4")]
        match self.try_(|p| p.accept_ipv4()) {
            Some(ipv4) => return Ok(IpAddress::Ipv4(ipv4)),
            None => ()
        }

        #[cfg(feature = "proto-ipv6")]
        match self.try_(|p| p.accept_ipv6(false)) {
    ...
    fn accept_ip_endpoint(&mut self) -> Result<IpEndpoint> {
        #[cfg(feature = "proto-ipv4")]
        match self.try_(|p| p.accept_ipv4_endpoint()) {
            Some(ipv4) => return Ok(ipv4),
            None => ()
        }

        #[cfg(feature = "proto-ipv6")]
        match self.try_(|p| p.accept_ipv6_endpoint()) {
