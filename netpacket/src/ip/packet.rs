...
use super::{Codepoint, Options, ToS};
...
pub struct Ipv4Packet<'a, 'b> {
    ...
    /// header and data.
    /// The minimum size is 20 bytes (header without data) and the maximum is
    /// 65,535 bytes.
    /// All hosts are required to be able to reassemble datagrams of size up to
    /// 576 bytes,
...
impl<'a, 'b> Ipv4Packet<'a, 'b> {
    ...
    /// [Protocol
    /// number](https://www.iana.
    /// org/assignments/protocol-numbers/protocol-numbers.xhtml)
...
impl<'a> Ipv6Packet<'a> {
    ...
    pub fn from_bytes(payload: &[u8]) -> Result<Self, ::std::io::Error> {
        ...
        let src_ip6 = ::std::net::Ipv6Addr::new(
            BigEndian::read_u16(&payload[8..10]),
            BigEndian::read_u16(&payload[10..12]),
            BigEndian::read_u16(&payload[12..14]),
            BigEndian::read_u16(&payload[14..16]),
            BigEndian::read_u16(&payload[16..18]),
            BigEndian::read_u16(&payload[18..20]),
            BigEndian::read_u16(&payload[20..22]),
            BigEndian::read_u16(&payload[22..24]),
        );
        let src_ip: u128 = u128::from(src_ip6);

        let dst_ip6 = ::std::net::Ipv6Addr::new(
            BigEndian::read_u16(&payload[24..26]),
            BigEndian::read_u16(&payload[26..28]),
            BigEndian::read_u16(&payload[28..30]),
            BigEndian::read_u16(&payload[30..32]),
            BigEndian::read_u16(&payload[32..34]),
            BigEndian::read_u16(&payload[34..36]),
            BigEndian::read_u16(&payload[36..38]),
            BigEndian::read_u16(&payload[38..40]),
        );
...
impl<'a, 'b> Packet<'a, 'b> {
    pub fn from_bytes(payload: &[u8]) -> Result<Self, ::std::io::Error> {
        let offset: usize;
        if cfg!(target_os = "macos") && (&payload[0..4] == &[0, 0, 0, 2] || &payload[0..4] == &[0, 0, 0, 10]) {
        ...
        match ver {
            ...
            4u8 => {
                match Ipv4Packet::from_bytes(&payload[offset..]) {
                    Ok(packet) => Ok(Packet::V4(packet)),
                    Err(e) => Err(e),
                }
            }
            6u8 => {
                match Ipv6Packet::from_bytes(&payload[offset..]) {
    ...
    pub fn tcp_ip_checksum(&self) -> bool {
        // https://en.wikipedia.
        // org/wiki/Transmission_Control_Protocol#Checksum_computation
        #[derive(Debug, PartialEq, Eq)]
